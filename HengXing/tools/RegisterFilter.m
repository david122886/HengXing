...
+(void)verifyUserName:(NSString*)userName withFinshed:(void(^)(BOOL success,NSString *errorMsg))block{
    ...
    NSRange range = [userName rangeOfString:@"^[\u4e00-\u9fa5]{2,8}$" options:NSRegularExpressionSearch];
    if (range.location == NSNotFound) {
        if(block)block(NO,@"用户名格式不正确");
        return;
    }
...
+(void)verifyAccount:(NSString*)account withFinshed:(void(^)(BOOL success,NSString *errorMsg))block{
    ...
    NSRange range = [account rangeOfString:@"^[A-Za-z0-9]{6,12}$" options:NSRegularExpressionSearch];
    if (range.location == NSNotFound) {
        if(block)block(NO,@"账号格式不正确");
        return;
    }
    if(block)block(YES,nil);
}
...
+(void)verifyPwd:(NSString*)pwd withFinshed:(void(^)(BOOL success,NSString *errorMsg))block{
    ...
    NSRange range = [pwd rangeOfString:@"^[A-Za-z0-9]{6,12}$" options:NSRegularExpressionSearch];
    if (range.location == NSNotFound) {
        if(block)block(NO,@"密码格式不正确");
        return;
    }
    if(block)block(YES,nil);
}
